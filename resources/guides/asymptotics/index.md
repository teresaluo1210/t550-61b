...
power of $2$. You can solve this using the same steps as above. Talk to a friend if you're not sure. 

<details>
<summary>Click this to see the solution.</summary>
<br>
<br>
Following the hint above, let's write each number as a power of 2:

$$2^1, 2^2, 2^4, 2^8, 2^{16}, 2^{32}, \ldots, N$$
...
Let $k$ be the index of the last element in the sequence (i.e. the sequence has $k$ elements). Then the last element is:

$$f(k) = 2^{2^k}$$

Now we have two ways to express the last term in the sequence, so we set them equal to each other:
...
$$\log_2(N) = \log_2(2^{2^k}) = 2^k$$

Then take the log of both sides again:

$$\log_2(\log_2(N)) = \log_2(2^k) = k$$

And we have our answer for the number of terms in the sequence:
